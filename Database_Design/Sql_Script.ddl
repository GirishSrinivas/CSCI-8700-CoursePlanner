...
ALTER TABLE section ADD CONSTRAINT section_pk PRIMARY KEY ( term,section_id,year );
...
--  ERROR: VARCHAR2 size not specified 
    ,
    login_net_id    VARCHAR2(25) NOT NULL
);

CREATE UNIQUE INDEX student__idx ON
    student ( login_net_id ASC );
...
ALTER TABLE enrolls ADD CONSTRAINT enrolls_section_fk FOREIGN KEY ( section_term,section_section_id,year )
    REFERENCES section ( term,section_id,year )
        ON DELETE CASCADE;
...
ALTER TABLE login ADD CONSTRAINT login_student_fk FOREIGN KEY ( student_nuid )
    REFERENCES student ( nuid )
        ON DELETE CASCADE;
...
ALTER TABLE student ADD CONSTRAINT student_login_fk FOREIGN KEY ( login_net_id )
    REFERENCES login ( net_id )
        ON DELETE CASCADE;
